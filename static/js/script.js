...
document.getElementById('districts-m').addEventListener('change', function() {
    var selectedOption = this.value;
    fetch('/process_data', {
        method: 'POST',
        headers: {
    ...
    .then(response => response.json())
    .then(data => {
        var secondDropdown = document.getElementById('places-m');
        secondDropdown.innerHTML = '                    <option value="">All</option>'; // Clear previous options
        data.forEach(option => {
            var optionElement = document.createElement('option');
            optionElement.textContent = option;
...
document.getElementById('districts-m').addEventListener('change', function() {
    ...
    .then(response => response.json())
    .then(data => {
        var secondDropdown = document.getElementById('crimeType-m');
        secondDropdown.innerHTML = '                    <option value="">All</option>'; // Clear previous options
        data.forEach(option => {
            var optionElement = document.createElement('option');
            optionElement.textContent = option;
...
document.getElementById('places-m').addEventListener('change', function() {
    var selectedOption = this.value;
    fetch('/process_crimeType_data2', {
        method: 'POST',
        headers: {
...
document.getElementById("triggerButton0-m").addEventListener("click", function() {
...
document.getElementById("triggerButton1-m").addEventListener("click", function() {
    // Make an AJAX request to the Flask route
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/vitcimAnalysis", true);
    xhr.onload = function () {
...
document.getElementById("triggerButton2-m").addEventListener("click", function() {
    // Make an AJAX request to the Flask route
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "/TimeSeries", true);
    xhr.onload = function () {
